...
from Datasets import aio_dataloader_burn as dl
...
def training(settings):
    ...
    # clip_models = ["Transformer_Stage", "Transformer_NA"]
    ...
    # build dataloader
    dataset_train = dl.IntpDataset(settings=settings, mask_distance=-1, burn_rate=burn_rate, call_name='train')
    ...
    for mask_distance in [0, ]:
        this_dataset = dl.IntpDataset(settings=settings, mask_distance=mask_distance, burn_rate=burn_rate, call_name='test')
...
def evaluate(settings):
    ...
    for mask_distance in [0, 20, 50]:
        this_dataset = dl.IntpDataset(settings=settings, mask_distance=mask_distance, burn_rate=burn_rate, call_name='eval')
